...
http://datadryad.org/bitstream/handle/10255/dryad.112108/Vincent_and_Schnell_2016.db.zip?sequence=1 
...
## getting interpro domain information 
split interpro annotation into R-readable files:
split -l 10000000 /Users/vitalii/Downloads/protein2ipr.dat /Users/vitalii/Downloads/interpro/interpro
...
##=========================================================##
``` {r hydropathy_vs_NCPR, fig.width = 12, fig.height = 12}
## Calculating protein median protein mass for each group
library("MASS")

ggplot(for_plot, aes(y = Hydropathy_m, x =NCPR, color = two_hybrid_vs_AP_MS)) +
  geom_point(alpha =0.7) +
#  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
#                labels = trans_format("log10", math_format(10^.x))) +
#  scale_x_discrete(labels = plot_labels)+
#  geom_abline(slope = 0, intercept = Mass_median, alpha =0.1) +
  ylab("mean hydropathy, log2")+scale_y_continuous(trans = "log2")+
  geom_smooth(method = "rlm")+
  xlab("net charge per residue")+
  ggtitle("What is the relationship between hydropathy and net charge per residue?",
          subtitle = "mean hydropathy log2 transformed vs net charge per residue")#+facet_grid(two_hybrid_vs_AP_MS~Organism)
```

### What is the relationship between hydropathy and the fraction of charged residues (CIDER)? 
##=========================================================##
``` {r hydropathy_vs_FCR, fig.width = 12, fig.height = 12}
## Calculating protein median protein mass for each group
library("MASS")

ggplot(for_plot, aes(y = Hydropathy_m, x =FCR, color = two_hybrid_vs_AP_MS)) +
  geom_point(alpha =0.7) +
#  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
#                labels = trans_format("log10", math_format(10^.x))) +
#  scale_x_discrete(labels = plot_labels)+
#  geom_abline(slope = 0, intercept = Mass_median, alpha =0.1) +
  ylab("mean hydropathy, log2")+scale_y_continuous(trans = "log2")+
  geom_smooth(method = "rlm")+
  xlab("the fraction of charged residues (CIDER)")+
  ggtitle("What is the relationship between hydropathy and the fraction of charged residues (CIDER)?",
          subtitle = "mean hydropathy log2 transformed vs \n the fraction of charged residues (CIDER)")#+facet_grid(two_hybrid_vs_AP_MS~Organism)
```

### What is the relationship between hydropathy and amino acid charge segregation? 
##=========================================================##
``` {r hydropathy_vs_Kappa, fig.width = 12, fig.height = 12}
## Calculating protein median protein mass for each group
library("MASS")

ggplot(for_plot, aes(y = Hydropathy_m, x =Kappa, color = two_hybrid_vs_AP_MS)) +
  geom_point(alpha =0.7) +
#  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
#                labels = trans_format("log10", math_format(10^.x))) +
#  scale_x_discrete(labels = plot_labels)+
#  geom_abline(slope = 0, intercept = Mass_median, alpha =0.1) +
  ylab("mean hydropathy, log2")+scale_y_continuous(trans = "log2")+
  geom_smooth(method = "rlm")+
  xlab("amino acid charge segregation (CIDER)")+
  ggtitle("What is the relationship between hydropathy and amino acid charge segregation?",
          subtitle = "mean hydropathy log2 transformed vs \n amino acid charge segregation (CIDER)")#+facet_grid(two_hybrid_vs_AP_MS~Organism)
```
...
```{r disorder_pca, fig.width=10, fig.height=7}
library(data.table)
proteome_vs_imex_interaction_details_f_Percent_disorder = as.data.table(proteome_vs_imex_interaction_details_f_Percent_disorder)
x =colnames(proteome_vs_imex_interaction_details_f_Percent_disorder)
whole_proteome_IDs = proteome_vs_imex_interaction_details_f_Percent_disorder[,whole_proteome_IDs] 
two_hybrid_vs_AP_MS = proteome_vs_imex_interaction_details_f_Percent_disorder[,two_hybrid_vs_AP_MS]

proteome_vs_imex_interaction_details_f_Percent_disorder_sel = as.matrix(proteome_vs_imex_interaction_details_f_Percent_disorder[,.(log10(Mass),IUPRED_PD/100,COILS_PD/100,HOTLOOPS_PD/100,REM465_PD/100,ICHR_consensus_PD/100,Frac_DP,Hydropathy_m,Hydropathy_U,FCR,NCPR,Kappa,Delta,Delta_max,(Proline_content -median(Proline_content))/mad(Proline_content))])
colnames(proteome_vs_imex_interaction_details_f_Percent_disorder_sel) = c("log10_Mass","IUPRED_PD","COILS_PD","HOTLOOPS_PD","REM465_PD","ICHR_consensus_PD","Frac_DP","Hydropathy_m","Hydropathy_U","FCR","NCPR","Kappa","Delta","Delta_max","norm_Proline_content")
rownames(proteome_vs_imex_interaction_details_f_Percent_disorder_sel) = whole_proteome_IDs
head(proteome_vs_imex_interaction_details_f_Percent_disorder_sel)
s = svd(t(proteome_vs_imex_interaction_details_f_Percent_disorder_sel))

U = s$u
D = s$d
V = s$v
str(D[1:2])
plot(D, main = "Variance explained", ylab = "Variance", xlab = "dimension")
PC1 = D[1]*V[,1]
PC2 = D[2]*V[,2]
PC = V[,1:2]%*%diag(D[1:2]) #!!!
all.equal(PC1, PC[,1])

proteome_vs_imex_interaction_details_f_Percent_disorder_PC = cbind(proteome_vs_imex_interaction_details_f_Percent_disorder,PC1,PC2)
all.equal(t(proteome_vs_imex_interaction_details_f_Percent_disorder_sel) == U%*%diag(D)%*%t(V))

ggplot(proteome_vs_imex_interaction_details_f_Percent_disorder_PC, aes(y = PC2, x =PC1, color = two_hybrid_vs_AP_MS)) +
  geom_point(alpha =0.7) +
#  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
#                labels = trans_format("log10", math_format(10^.x))) +
#  scale_x_discrete(labels = plot_labels)+
#  geom_abline(slope = 0, intercept = Mass_median, alpha =0.1) +
  ...
  xlab("PC2")+
  ggtitle("PCA",
          subtitle = "") + facet_grid(.~two_hybrid_vs_AP_MS)
