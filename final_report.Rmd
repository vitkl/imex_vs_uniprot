...
BioGRID database is the second major primary protein interaction database which when combined with IntAct contain all interactions information which has been curated to databases. BioGRID is characterised by shallow curation level (retains very little information about the experiment) and it identifies proteins using Entrez Gene ID while IntAct uses UniprotKB identifiers. This introduces additional mapping step
...
## 4. Major protein interaction detection methods (two-hybrid and AP-MS) exhibit a bias towards biochemical properties of the proteins involved?
...
```{r interaction_types_combine_and_plot,  fig.width = 10, fig.height = 10, fig.cap= "Figure 6"}
...
```{r protein_length_det_method_rlm,  fig.width = 10, fig.height = 10, fig.cap= "Figure 7"}
...
## 5. What is the relationship between the number of publications and the number of interactions (or MI score) ?
...
all.IntAct.proteins_scale = tidyr::spread(all.IntAct.proteins_scale_l, key = large_scale, value = large_scale, fill = 0)
...
```{r N_publications_vs_N_interactions, fig.height=10, fig.width=10, fig.cap= "Figure 8"}
...
ggplot(all.IntAct.proteins, aes(interactor_total_PMID, unique_interactions_per_interactor)) +  scale_x_log10() + scale_y_log10() +
    ...
    ylab("the number of interactions(per protein) in which proteins encoded by that gene participate") + xlab("the number of publications per gene") + geom_smooth(method = "rlm", se = T) +
  facet_grid(SPECIES_NAME~large_small_scale, scales = "free_y") + geom_point(size = 0.5, alpha = 0.1) +
    theme(strip.text.y = element_text(angle = 0)) + geom_smooth(method = "lm", se = T, aes(color = "red"))+geom_text(aes(label=IDs_interactor), hjust=0, vjust=0)
}
```

```{r publications_vs_MI_score, fig.height=10, fig.width=10, fig.cap= "Figure 9"}
library(MASS)
...
```
  ...
  You can clearly see that the more studied overall the gene is (the more publications per gene there is) the more interactions proteins encoded by that gene tend to have and the more evidence there is for those interactions. Which is not surprising because articles tend to publish novel interactions and the more studies there is overall - the more studies look into protein interactions. What is quite surprising is that large scale experiments also exhibit this trend.
...
{
# tagging interactons
all.IntAct.modif[,large_scale := as.numeric(interaction_ids_per_Publication >= Ns[i])]
all.IntAct.modif[,small_scale := as.numeric(interaction_ids_per_Publication < Ns[i])]
}
# interactions to proteins and genes
## tagging interactors by the scale of the study
{
...
# Select only human, only uniprotkb identifiers, only large scale studies
  ...
  coefs[i,] = c(nrow(all.IntAct.proteins),length(unique(all.IntAct.proteins$IDs_interactor)))#rlm_res$coefficients
...
```{r interaction_properties_vs_pubN, fig.height=10, fig.width=12, fig.cap= "Figure 10"}
...
all.IntAct.protein_interaction_details_all = all.IntAct.protein_interaction_details_all[two_hybrid_vs_AP_MS != "not in IMEx" & SPECIES_NAME %in% c("Homo sapiens", "Mus musculus", "S. cerevisiae, strain S288c", "Escherichia coli, strain K12"),]
{
...
You can notice that lines for different methods are located at different heights, which tells us that different methods identify different number of interactions across both large and small scale studies. The plot below shows the distribution of number of interactions (from large/small/both-scale studies and interaction detection methods) per protein. (plot failed for some reason - Quitting from lines 1157-1167 (final_report.Rmd) 
...
```{r interaction_properties_vs_unique interactors, fig.height=10, fig.width=12}
       # read the saved table with human_not_in_IMEx_protein_properties_processing chunk
...
### interaction information is not available for proteins with lower fraction of intrinsically disordered domains
