...
6. Are proteins with high fraction of intrinsically disordered domains more likely to have interactions available and do they have more interactions (if normalized for how well-studied proteins are)?
...
all.IntAct[, Interaction_identifiers := unlist(gsubfn::strapplyc(Interaction_identifiers,"EBI-[[:digit:]]+",simplify = T))]
}
...
This difference in protein mass between proteins present and absent in the interactome is highly unlikely to occur by chance (Wilcox rank test on the whole population of proteins, Monte-Carlo sampling (is it useful?), permutation of labels followed by Wilcox rank test - Supplementary figure 3,4,5). Removing `r n_odor_receptors` olfactory receptors does not change this trend.
...
```{r human_not_in_IMEx_proteins_are_shorter_Wilcox}
      ...
      wilcox.test.r = wilcox.test(log10(SwissProt_not_IMEX), log10(SwissProt_and_IMEX), conf.int = T)
...
### Most of the human proteins with no available interactions (from IMEx) are membrane proteins

### If a protein is missing protein evidence in Uniprot it is also more likely to be missing from IntAct

```{r not_in_imex_missing_protein_evidence}

```

## 4. Do major protein interaction detection methods (two-hybrid and AP-MS) exhibit any bias towards biochemical properties of the proteins involved?
...
```{r interaction_types_combine_and_plot,  fig.width = 10, fig.height = 10}
...
fit_rlm = rlm(log10(Mass)~two_hybrid_vs_AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == sp,])
coefs <- coef(fit_rlm)
...
boxplot(log10(Mass)~two_hybrid_vs_AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == sp,], 
           vertical=TRUE, pch=1, method="jitter",las=1,cex.axis= 0.70, main = paste0(sp, ": protein length vs interaction detmethod, rlm"), cex.main=0.99)
abline(h=0)
...
arrows(5+a,coefs[1],5+a,coefs[1]+coefs[5],lwd=3,col=cols[5],length=lgth)
segments(5+2*a,coefs[1]+coefs[5],5-2*a,coefs[1]+coefs[5],col=cols[2])
# two_hybrid vs AP_MS significance testing
C <- matrix(c(0,0,1,0,-1),1)
two_hybridvsAP_MS <- glht(fit_rlm, linfct=C)
Sigma.hat = two_hybridvsAP_MS$vcov
p_val = pnorm((coefs[3] - coefs[5])/sqrt(C %*% Sigma.hat %*% t(C)),lower.tail = F)*2
arrows(3,coefs[1]+coefs[3]-0.3,5,coefs[1]+coefs[5]-0.3,lwd=3,col=cols[6],length=lgth)
text(4,(coefs[1]+coefs[3]-0.35 + coefs[1]+coefs[5]-0.35)/2,paste("p-value: ",signif(p_val,2)),cex =0.9)
# two_hybrid vs AP_MS_and_two_hybrid significance testing
C <- matrix(c(0,0,0,1,-1),1)
two_hybridvsAP_MS <- glht(fit_rlm, linfct=C)
Sigma.hat = two_hybridvsAP_MS$vcov
p_val = pnorm((coefs[4] - coefs[5])/sqrt(C %*% Sigma.hat %*% t(C)),lower.tail = F)*2
arrows(4,coefs[1]+coefs[4]+0.3,5,coefs[1]+coefs[5]+0.3,lwd=3,col=cols[6],length=lgth)
text(4.5,(coefs[1]+coefs[4]+0.35 + coefs[1]+coefs[5]+0.35)/2,paste("p-value: ",signif(p_val,2)),cex =0.9)
# other_methods vs AP_MS significance testing
C <- matrix(c(0,-1,1,0,0),1)
two_hybridvsAP_MS <- glht(fit_rlm, linfct=C)
Sigma.hat = two_hybridvsAP_MS$vcov
p_val = pnorm((coefs[3] - coefs[2])/sqrt(C %*% Sigma.hat %*% t(C)),lower.tail = F)*2
arrows(3,coefs[1]+coefs[3]-0.4,2,coefs[1]+coefs[2]-0.4,lwd=3,col=cols[6],length=lgth)
text(2.5,(coefs[1]+coefs[3]-0.45 + coefs[1]+coefs[2]-0.45)/2,paste("p-value: ",signif(p_val,2)),cex =0.9)
# other_methods vs AP_MS_and_two_hybrid significance testing
C <- matrix(c(0,-1,0,1,0),1)
two_hybridvsAP_MS <- glht(fit_rlm, linfct=C)
Sigma.hat = two_hybridvsAP_MS$vcov
p_val = pnorm((coefs[4] - coefs[2])/sqrt(C %*% Sigma.hat %*% t(C)),lower.tail = F)*2
arrows(4,coefs[1]+coefs[4]+0.4,2,coefs[1]+coefs[2]+0.4,lwd=3,col=cols[6],length=lgth)
text(3,(coefs[1]+coefs[4]+0.45 + coefs[1]+coefs[2]+0.45)/2,paste("p-value: ",signif(p_val,2)),cex =0.9)

legend("bottomright",names(coefs),fill=cols,cex=.75,bg="white")
}
# ============================================
# IMEx_two_hybrid_AP_MS - plain linear model
proteome_vs_imex_interaction_details_all[,IMEx_two_hybrid_AP_MS := paste0(IMEx,two_hybrid,AP_MS)]
library(MASS)
fit_lm = lm(log10(Mass)~two_hybrid_vs_AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == "Homo sapiens",])
X = model.matrix(log10(Mass)~two_hybrid_vs_AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == "Homo sapiens",])
summary(fit_lm)
library(rafalib)
library(multcomp)
library(MASS)
...
{
boxplot(log10(Mass)~two_hybrid_vs_AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == "Homo sapiens",], 
           vertical=TRUE, pch=1, method="jitter",las=1,cex.lab   = 0.65, main = "How does protein interaction detection method depend on the protein length? lm")
abline(h=0)
arrows(1+a,0,1+a,coefs[1],lwd=3,col=cols[1],length=lgth)
abline(h=coefs[1],col=cols[1])
#IMEx
...
C <- matrix(c(0,0,0,1,-1),1)
two_hybridvsAP_MS <- glht(fit_lm, linfct=C)
summary(two_hybridvsAP_MS)
coefs[4] - coefs[3]

#t(C) %*% coefs
#Sigma.hat <- sum(fit$residuals^2)/(nrow(X) - ncol(X)) * solve(t(X) %*% X)
#signif(Sigma.hat, 2)
#sqrt(t(C) %*% Sigma.hat %*% C)

# ============================================
# Draft model
# log10(Mass)~IMEx*two_hybrid*AP_MS
proteome_vs_imex_interaction_details_all[,IMEx_two_hybrid_AP_MS := paste0(IMEx,two_hybrid,AP_MS)]
fit = lm(log10(Mass)~IMEx*two_hybrid*AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == "Homo sapiens",])
X = model.matrix(log10(Mass)~IMEx*two_hybrid*AP_MS, data = proteome_vs_imex_interaction_details_all[SPECIES_NAME == "Homo sapiens",])
summary(fit)
...
cols <- brewer.pal(8,"Dark2")
{
boxplot(split(log10(proteome_vs_imex_interaction_details_all$Mass), proteome_vs_imex_interaction_details_all$IMEx_two_hybrid_AP_MS, drop = TRUE), 
...
#IMEx
arrows(2+a,coefs[1],2+a,coefs[1]+coefs[2],lwd=3,col=cols[2],length=lgth)
arrows(3+a,coefs[1],3+a,coefs[1]+coefs[2],lwd=3,col=cols[2],length=lgth)
arrows(4+a,coefs[1],4+a,coefs[1]+coefs[2],lwd=3,col=cols[2],length=lgth)
arrows(5+a,coefs[1],5+a,coefs[1]+coefs[2],lwd=3,col=cols[2],length=lgth)
segments(2+2*a,coefs[1]+coefs[2],5-2*a,coefs[1]+coefs[2],col=cols[2])
mean100 = proteome_vs_imex_interaction_details_all[IMEx_two_hybrid_AP_MS == "100", mean(log10(Mass))]
segments(2+2*a,mean100,2-2*a,mean100,col=cols[8])
#two-hybrid
arrows(4,coefs[1]+coefs[2],4,coefs[1]+coefs[2]+coefs[3],lwd=3,col=cols[3],length=lgth)
arrows(5,coefs[1]+coefs[2],5,coefs[1]+coefs[2]+coefs[3],lwd=3,col=cols[3],length=lgth)
segments(4+2*a,coefs[1]+coefs[2]+coefs[3],5-2*a,coefs[1]+coefs[2]+coefs[3],col=cols[3])
#AP_MS
arrows(3-a,coefs[1]+coefs[2],3-a,coefs[1]+coefs[2]+coefs[4],lwd=3,col=cols[4],length=lgth)
segments(3+2*a,coefs[1]+coefs[2]+coefs[4],3-2*a,coefs[1]+coefs[2]+coefs[4],col=cols[4])
arrows(5-a,coefs[1]+coefs[2]+coefs[3],5-a,coefs[1]+coefs[2]+coefs[3]+coefs[4],lwd=3,col=cols[4],length=lgth)
segments(5+2*a,coefs[1]+coefs[2]+coefs[3]+coefs[4],5-2*a,coefs[1]+coefs[2]+coefs[3]+coefs[4],col=cols[4])
#two-hybrid and #AP_MS
arrows(5-2*a,coefs[1]+coefs[2]+coefs[4]+coefs[3],5-2*a,coefs[1]+coefs[2]+coefs[3]+coefs[7]+coefs[4],lwd=3,col=cols[5],length=lgth)
segments(5+2*a,coefs[1]+coefs[2]+coefs[3]+coefs[7]+coefs[4],5-2*a,coefs[1]+coefs[2]+coefs[3]+coefs[7]+coefs[4],col=cols[5])
legend("bottomright",names(coefs),fill=cols,cex=.75,bg="white")
}
mean111 = proteome_vs_imex_interaction_details_all[IMEx_two_hybrid_AP_MS == "111", mean(log10(Mass))]
coefs[1]+coefs[2]+coefs[3]+coefs[7]+coefs[4]

#two_hybrid vs AP_MS test
#library(multcomp) ##Available from CRAN
#C <- matrix(c(0,0,1,-1,0,0,0,0),1)
#two_hybridvsL2AP_MS <- glht(fit, linfct=C)
#summary(two_hybridvsL2AP_MS)
#coefs[4] - coefs[3]
...
## 5. What is the relationship between the number of interactions or MI score and the number of publications?
...
#all.IntAct.modif[Expansion_methods != "psi-mi:\"MI:1060\"(spoke expansion)",
#                 unique_interactions_per_Publication_nospoke := length(unique(unique_interactions)),
#                 by = Publication_Identifiers]
#all.IntAct.modif[Expansion_methods != "psi-mi:\"MI:1060\"(spoke expansion)",
#                 interaction_ids_per_Publication_nospoke := length(unique(Interaction_identifiers)),
#                 by = Publication_Identifiers]

# selecting large scale / small scale threshold 
library(rafalib)
mypar(1,2)
plot(log10(all.IntAct.modif$unique_interactions_per_Publication), 
     log10(all.IntAct.modif$interaction_ids_per_Publication),
...
mypar(1,2)
hist(log10(all.IntAct.modif$unique_interactions_per_Publication),100)
abline(v = 2.55)
hist(log10(all.IntAct.modif$interaction_ids_per_Publication),100)
abline(v = 2.55)
#hist(log10(all.IntAct.modif$unique_interactions_per_Publication_nospoke),100)
#abline(v = 2.55)
#hist(log10(all.IntAct.modif$interaction_ids_per_Publication_nospoke),100)
#abline(v = 2.55)
}
...
{
# tagging interactons
all.IntAct.modif[,large_scale := as.numeric(interaction_ids_per_Publication > 10^2.55)]
all.IntAct.modif[,small_scale := as.numeric(interaction_ids_per_Publication <= 10^2.55)]
}
...
all.IntAct.modif[,unique_interactions_isoforms := apply(data.table(IDs_interactor_A_isoforms,IDs_interactor_B_isoforms,stringsAsFactors = F), 1,
                                               function(a) { z = sort(a)
                                               paste0(z[1],"_",z[2]) })]
#==================== ????? how do I properly split interactions into large/small scale?
all.IntAct.modif[large_scale == 1,unique_interactions_large_scale := apply(data.table(IDs_interactor_A,IDs_interactor_B,large_scale ,stringsAsFactors = F), 1,
                                               function(a) { z = sort(a[1:2])
                                               paste0(z[1],"_",z[2],"_l",a[3]) })]
all.IntAct.modif[small_scale == 1,unique_interactions_small_scale := apply(data.table(IDs_interactor_A,IDs_interactor_B,large_scale ,stringsAsFactors = F), 1,
                                               function(a) { z = sort(a[1:2])
                                               paste0(z[1],"_",z[2],"_l",a[3]) })]
# all.IntAct.modif[large_scale == 1,unique_interactions_isoforms_large_scale := apply(data.table(IDs_interactor_A_isoforms,IDs_interactor_B_isoforms,large_scale ,stringsAsFactors = F), 1,
#                                               function(a) { z = sort(a[1:2])
#                                               paste0(z[1],"_",z[2],"_l",a[3]) })]
# all.IntAct.modif[small_scale == 1,unique_interactions_isoforms_small_scale := apply(data.table(IDs_interactor_A_isoforms,IDs_interactor_B_isoforms,large_scale ,stringsAsFactors = F), 1,
#                                               function(a) { z = sort(a[1:2])
#                                               paste0(z[1],"_",z[2],"_l",a[3]) })]
#====================
  #UniprotID
...
all.IntAct.modif[,unique_interactions_per_interactor_A_large_scale := length(unique(grep(IDs_interactor_A, unique_interactions_large_scale ,value = T))), by = IDs_interactor_A]
all.IntAct.modif[,unique_interactions_per_interactor_B_large_scale := length(unique(grep(IDs_interactor_B, unique_interactions_large_scale ,value = T))), by = IDs_interactor_B]
all.IntAct.modif[,unique_interactions_per_interactor_A_small_scale := length(unique(grep(IDs_interactor_A, unique_interactions_small_scale ,value = T))), by = IDs_interactor_A]
all.IntAct.modif[,unique_interactions_per_interactor_B_small_scale := length(unique(grep(IDs_interactor_B, unique_interactions_small_scale ,value = T))), by = IDs_interactor_B]
  #GeneID
...
all.IntAct.proteins = all.IntAct.modif[,data.table(
                   ...
                   unique_interactions_per_interactor_large_scale = c(unique_interactions_per_interactor_A_large_scale, unique_interactions_per_interactor_B_large_scale),
                   unique_interactions_per_interactor_small_scale = c(unique_interactions_per_interactor_A_small_scale, unique_interactions_per_interactor_B_small_scale)
                   )]
...
fit = lm(Median_MI_score~interactor_total_PMID*large_small_scale, data=all.IntAct.proteins)
summary(fit)
...
ggplot(all.IntAct.proteins, aes(interactor_total_PMID, unique_interactions_per_interactor))+
    ...
    ylab("the number of interactions(per protein) in which proteins encoded by that gene participate") + xlab("the number of publications per gene") +
     geom_smooth(method = "rlm", se = T)+
  facet_grid(SPECIES_NAME~large_small_scale, scales = "free") + geom_point(size = 0.1, alpha = 0.05)+
    theme(strip.text.y = element_text(angle = 0))
}
```

## 6. Are proteins with high fraction of intrinsically disordered domains more likely to have interactions available
...
```{r Supplementary2_BioGRID_vs_IMEx_vs_Uniprot_N_Uniprot&Species, cache=F, fig.width=8, fig.height=8, fig.cap="Supplementary figure 2" }
...
```{r Supplementary3_protein_mass_distribution_qqplot, fig.width=8, fig.height=8, fig.cap="Supplementary figure 3. The distribution of the logarhythm base 10 of protein mass is approximately normal"}
...
```{r Supplementary4_protein_mass_distribution_monte_carlo, fig.width=8, fig.height=8, fig.cap="Supplementary figure 4. Monte-Carlo sampling can pick up the difference in protein mass between proteins present and missing from IntAct"}
...
```{r Supplementary4_protein_mass_distribution_permutations, fig.width=8, fig.height=8, fig.cap="Supplementary figure 4. Monte-Carlo sampling can pick up the difference in protein mass between proteins present and missing from IntAct"}
