...
6. Are proteins with higher fraction of intrinsically disordered domains more likely to have interactions available and do they have more interactions (if normalized for how well-studied proteins are)?
...
### Available interactome covers substantial fraction of the reviewed proteome of main model species
   ...
   In this section we compare the list of all proteins (per species) in Uniprot (reviewed part (or SwissProt) - Figure 2, reviewed and unreviewed parts (or all UniprotKB: SwissProt + trEMBL) - Supplementary Figure 1) to the list of protein which have interaction partners annotated in IMEx consortium databases. 
...
```
   ...
   A large fraction of human, mouse, arabidopsis proteins-interactors and more than a half of drosophila and *C.elegans* proteins-interactors are absent in SwissProt (but included in trEMBL) – suggesting under-annotation by SwissProt-Uniprot team.
...
```
   ...
   On the other hand, some researchers advocate that interactions important for cellular function should be conserved between species [].
...
## 3. Which proteins are missing interaction evidence?
...
The distributions of protein mass has a very long right tail - there is much more big proteins than normal distribution would predict (Supplementary figure 3), which only allows to use non-parametric statistical tests (Wilcox test). Log10 transformation of protein mass, though, makes extreme values less extreme and is approximately normally distributed.
...
n_odor_receptors = length(unique(grep("Odor", proteome_vs_imex_details_f$Protein.names, value = F))) + length(unique(grep("Olfactory", proteome_vs_imex_details_f_minus_odor$Protein.names, value = F))) 
      ...
      ggplot(proteome_vs_imex_details_f_minus_odor_olf.c, aes(x = Mass, color = whole_proteome_Uniprot_IMEx, alpha =0.7, label = olfac)) +geom_freqpoly(bins = 100)+ scale_x_log10() + facet_grid(olfac~.) + theme(strip.text.y = element_text(angle = 0)) +xlab("protein mass, Da, log10 scale") + ggtitle("the distribution of protein mass for proteins with and \n without interacting partners annotated in IMEx")
```
...
Exprimental role of the protein (bait/prey/neutral component) can in theory be influenced by protein mass. For example, it may be easier to clone shorter proteins for use as bait/prey. So it important to see if exprimental role is really influenced by protein mass.
...
bait_vs_non_bait[,mean_mass_per_role := mean(Mass), by = bait_prey_status]
...
ggplot(bait_vs_non_bait, aes(x = Mass, color = bait_prey_status)) +geom_density(bins = 100)+ scale_x_log10() + facet_grid(bait_prey_status_label~.) + theme(strip.text.y = element_text(angle = 0)) +xlab("protein mass, Da, log10 scale") + ggtitle("the distribution density of protein mass by experimental role \n (for proteins in IMEx)")

bait_vs_non_bait = bait_vs_non_bait[N_bait_prey_status > 1000,]
ggplot(bait_vs_non_bait, aes(x = Mass, color = bait_prey_status_label)) +geom_density(bins = 100)+ scale_x_log10()  + theme(strip.text.y = element_text(angle = 0)) +xlab("protein mass, Da, log10 scale") + ggtitle("the distribution density of protein mass by experimental role \n (for proteins in IMEx)")

```

Evidently, experimental role of a protein is not substantially influenced by the mass of the protein.
...
## 4. Do major protein interaction detection methods (two-hybrid and AP-MS) exhibit a bias towards biochemical properties of the proteins involved?

   The problem of bias in interactomics is often discussed in research papes and in the community. Now we will focus on the bias in experimental detection method. We choose to compare two major families of methods: two-hybrid and affinity-purification-mass-spectrometry (AP-MS). We define two-hybrid using PSI-MI ontology: detection method = "transcriptional complementation assay"" - all methods which belong to this type (which are children terms in ontology). We define AP-MS using two PSI-MI ontology terms: detection method = "affinity chromatography technology" and participant identification method = "partial identification of protein sequence". 
   We get interations detected using these methods by querying PSICQUIC. We use word formulations rather than ontology term identifiers because PSICQUIC package in R is not equipped to handle quotes in the query text ("MI:0004" AND "MI:0102").

### AP-MS is biased towards longer proteins

   Unlike the experimental role type, whether a protein interaction is detected using a partucular interaction detection method is influenced by the mass of that protein.
...
```
   ...
   The statistical significance of the difference in protein mass across protein detection methods was tested using linear model approach. 

```{r protein_length_det_method_rlm,  fig.width = 10, fig.height = 8, fig.cap= "Figure 7"}
...
## 5. How does the number of interactions (or MI score) depend on the number of publications?
...
```{r N_publications_vs_N_interactions, fig.height=9, fig.width=9, fig.cap= "Figure 8"}
...
## 5. Proteins with no interacting partners are less studied overall
...
## 6. Are proteins with higher fraction of intrinsically disordered domains more likely to have interactions available?
...
ggplot(Percent_disorder_types_for_plot, aes(x = percent_disorder, color = whole_proteome_Uniprot_IMEx, label = SPECIES_NAME)) +geom_freqpoly(bins = 100)+  facet_grid(SPECIES_NAME~disorder_pred_algorithm) + theme(strip.text.y = element_text(angle = 0)) +xlab("fraction of disordered regions, 0...100") + ggtitle("the distribution of the fraction of disordered regions for proteins with \n and without interacting partners annotated in IMEx")

ggplot(Percent_disorder_types_for_plot, aes(x = percent_disorder, color = whole_proteome_Uniprot_IMEx, label = SPECIES_NAME)) +geom_density(bins = 100)+  facet_grid(SPECIES_NAME~disorder_pred_algorithm) + theme(strip.text.y = element_text(angle = 0)) +xlab("fraction of disordered regions, 0...100") + ggtitle("the distribution density of the fraction of disordered regions for proteins with \n and without interacting partners annotated in IMEx")
```
...
Proteins which don't have interacton evidence in IntAct tend to have lower fraction of charged residues and higher mean hydropathy. This correlates well with the GO (cellular component) enrichment result: these proteins are largely membrane proteins (this was done in cytoscape, not in R yet).


## 7. Do proteins with higher fraction of intrinsically disordered domains have more interacting partners?
...
```
   ...
   From these graphs we can conclude that the fraction of disordered domains does not depend on the number of studies.

   If disordered regions are often needed for molecular interactions we would expect that proteins with higher fraction of disordered regions will tend to have more interacting partners (irrespective of the study bias).
...
```
   ...
   As you can see from the loess-fitted curve (red, loess stands for local polynimial regression), the proteins with low fraction of disordered domains tend to have less interactions while further increasing the fraction of disordered regions doesn't change the trend. More general trend is shown in blue - robust linear regression. Whether that's the result of chance or study bias is yet to be tested.
...
if(!(length(list.files(filename_interpro2)) > 0)) {
  ...
  system(paste0("split -l 5000000 ", 
                filename_interpro1," ",
                filename_interpro2,"/"))
  # writing .gitignore for this files
  filename_interpro3 = list.files(filename_interpro2)
  if(file.exists(".gitignore")){
  gitignore = c(substr(filename_interpro1, 2, nchar(filename_interpro1)),
                paste0(substr(filename_interpro2, 2, nchar(filename_interpro2)), "/",filename_interpro3),
                readLines(".gitignore"))
  write(gitignore, ".gitignore")}
}
filename_interpro3 = list.files(filename_interpro2)
...
x = merge(proteome_vs_imex_interaction_details_f_Percent_disorder, domains[,unique(data.table(UniprotID = UniprotID, domains_per_protein = domains_per_protein))], by.x = "whole_proteome_IDs", by.y = "UniprotID", all = T)
...
```{r Supplementary2_BioGRID_vs_IMEx_vs_Uniprot_N_Uniprot&Species, cache=F, fig.width=8, fig.height=8, fig.cap="Supplementary figure 2. The number of interacting proteins from given species or the other species in BioGRID" }
