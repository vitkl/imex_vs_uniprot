...
```{r human_not_in_IMEx_protein_properties_GOenrichment, fig.width=10, fig.height=8}
##====================================================================
suppressPackageStartupMessages({
      library(data.table)
      library(qvalue)
...
enrichedGOterms_tab = as.data.table(as.data.frame(enrichedGOterms))
upsetplot(enrichedGOterms)
```

```{r human_not_in_IMEx_protein_properties_GOdepleted, fig.width=10, fig.height=8}
...
```{r GO_enrichment_map, fig.height=20, fig.width=20}
set.seed(7)
enrichMap(enrichedGOterms, n = 40)
```
...
proteome_vs_imex_details_f_minus_odor_olf.c[, median_Mass_per_group := median(log10(Mass)), by = whole_proteome_Uniprot_IMEx_x_olfac]
      ...
      ggplot(proteome_vs_imex_details_f_minus_odor_olf.c, aes(x = Mass, color = whole_proteome_Uniprot_IMEx, label = olfac)) +geom_freqpoly(bins = 100)+ scale_x_log10() + theme(strip.text.y = element_text(angle = 0)) +xlab("protein mass, Da, log10 scale") + ggtitle("the distribution of protein mass for proteins with and \n without interacting partners annotated in IMEx")+geom_vline(aes(xintercept = 10^median_Mass_per_group, color = whole_proteome_Uniprot_IMEx),alpha =0.7)  + facet_grid(olfac~.)
```
...
# fread("http://www.targetvalidation.org/api/latest/public/search?q=O00198&size=1&filter=target&format=tab")
...
# write(toJSON(list(target = "ENSG00000135116", fields = c("association_score.overall","target.id","disease.id"), format="tab")), "targetvalidation.txt")
# system(paste0("curl -X POST -d @targetvalidation.txt --header \"Content-Type:application/json\" http://www.targetvalidation.org/api/latest/public/association/filter"))
# library(ropentargets)
# geneName <- c('O00198', 'O00203')
# geneObj <- Gene$new(geneName)
# geneObj$getGeneIDNameMap() 

# manually checking for updated file is needed
 suppressPackageStartupMessages(library(jsonlite))
 open_targets_associations_filename = "./Data/16.12_association_data_fixed.json"
 if(!file.exists(open_targets_associations_filename)){
 open_targets_associations_url = "https://s3-eu-west-1.amazonaws.com/targetvalidation-dumps/16.12_association_data_fixed.json.gz"
 downloader::download(open_targets_associations_url,
                      paste0(open_targets_associations_filename, ".gz"))
 R.utils::gunzip(open_targets_associations_filename)
 }
 fromJSON(open_targets_associations_filename)
 open_targets_mapping = stream_in(file(open_targets_associations_filename), pagesize = 5000)
...
### DisGnet
      # mapping UniprotID to entrezgene id for DisGnet enrichment using enrichDGN() from DOSE
      #loading biomaRt functionality
library(biomaRt)
...
# mapping IDs
# ensembl_gene_id = getBM(attributes = c('ensembl_gene_id', 'uniprot_swissprot'), 
#              values = proteome_vs_imex_details_f$whole_proteome_IDs, 
#              mart = ensembl)
# ensembl_gene_id = ensembl_gene_id[ensembl_gene_id$uniprot_swissprot != "",]
# ensembl_gene_id_comp = ensembl_gene_id[complete.cases(ensembl_gene_id),]

entrezgene = getBM(attributes = c('entrezgene', 'uniprot_swissprot'), 
              values = proteome_vs_imex_details_f$whole_proteome_IDs, 
              mart = ensembl)
entrezgene = entrezgene[entrezgene$uniprot_swissprot != "",]
entrezgene_comp = entrezgene[complete.cases(entrezgene),]
# Mapping ID to the other info in the table
proteome_vs_imex_details_f = merge(x = proteome_vs_imex_details_f, y = entrezgene_comp, 
                                   by.x = "whole_proteome_IDs",
